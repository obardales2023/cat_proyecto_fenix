...
public JSONObject performLogic(JSONObject state, Map<String, String> additionalParams) throws Exception {
    ...
    SQLServerCallableStatement cstmt = null;
	 ...
	 try 
	 {
         ...
         // LLAMADA SP
         //cstmt = (SQLServerCallableStatement) con.prepareCall("{? = call dbo.SP_IVR_ConsultaOfertable(?)}");
         cstmt = (SQLServerCallableStatement) con.prepareCall("{? = call dbo.SP_IVR_ConsultaOfertable_NE(?,?)}");
         //cstmt = (SQLServerCallableStatement) con.prepareCall("{? = call dbo.SP_IVR_ConsultaOfertableEnruta(?,?,?)}");
		 ...
		 varOfertaDB = cstmt.getString(3);
		 //varPeso = cstmt.getString(4);
		 
         long stopTime = System.nanoTime();
		 long elapsedTime = (stopTime - startTime)/1000000;
		 LoggerMensaje += "Valor de Retorno de Ejecucion de SP:" + returnSP + " varOfertaDB[" + varOfertaDB + "]" + " varPeso[" + varPeso + "]";
		 LoggerMensaje += "JS SP_JSP_ConsultaOfertable.jsp tardo en ejecutarse:" + elapsedTime;
		...
		//FINALIZA TIMER
		 ...
		 result.put("varOfertaDB",varOfertaDB);
		 result.put("varPesoDB",varPeso);
