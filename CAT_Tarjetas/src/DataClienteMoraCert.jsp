...
public JSONObject performLogic(JSONObject state, Map<String, String> additionalParams) throws Exception {
	...
	try {
		...
		int idCuenta = Integer.parseInt(additionalParams.get("idCuenta"));
		String ultimosDigitos  = additionalParams.get("ultimosDigitos");
			
		
		result.put("HttpCode","-1");
		result.put("numero_tarjeta","-1");
		result.put("mora","-1");
		
		result.put ("vigencia","-1"); 
		result.put ("saldoPendiente","-1");
				
		System.out.println("WS[GetDataMoraCliente]> CallID: " + callid + ", URL: " + url + ", Token: " + token + ", SesionId: " + sesionid + " idCuenta: " + idCuenta + " ultimosDigitos: " + ultimosDigitos);
		MainRestWS clientWS = new MainRestWS(callid); 
		ConsultaActivosResponse respuestaOut = clientWS.ConsultaActivos(token, sesionid, idCuenta, ultimosDigitos, url);
